...
from dashboard.api_client import get_json, health_ok, resolve_base_url
...
def _load_tenants(base: str) -> list[dict]:
    data = get_json("/api/tenants", base_url=base)
    return list(data.get("items") or [])


def _resolve_tenant_id(base: str) -> str | None:
    """
    Un solo tenant: se usa su id automáticamente (sin selectbox).
    Varios: selectbox en sidebar.
    Cero: aviso y None.
    """
    tenants = _load_tenants(base)
    if not tenants:
...
def tab_overview(base: str, tenant_id: str | None) -> None:
    ...
    try:
        summary = get_json("/api/metrics/summary", params, base_url=base)
    ...
    try:
        series_data = get_json("/api/metrics/users-over-time", mparams, base_url=base)
    except Exception as e:
...
def tab_users(base: str, tenant_id: str | None) -> None:
    ...
    try:
        data = get_json("/api/users", params, base_url=base)
    except Exception as e:
    ...
    st.divider()
    try:
        detail = get_json(f"/api/users/{uid}", base_url=base)
    except Exception as e:
...
def tab_tenant_feed(base: str, tenant_id: str | None) -> None:
    ...
    try:
        data = get_json(f"/api/tenants/{tenant_id}/messages", params, base_url=base)
    except Exception as e:
...
def main() -> None:
    ...
    tenant_id = _resolve_tenant_id(base)

    tab1, tab2, tab3 = st.tabs(["Resumen", "Usuarios", "Feed"])
    with tab1:
        tab_overview(base, tenant_id)
    with tab2:
        tab_users(base, tenant_id)
    with tab3:
        tab_tenant_feed(base, tenant_id)
